...
const validBody = { name: "Phil Adams", email: "phil@example.com", message: "I'm a sole trader and need self-assessment help." };
...
describe("POST /api/contact — validation", () => {
  ...
  test("returns 400 when email is missing", async () => { const res = await POST(makeRequest({ name: "Phil", message: "Hi" })); expect(res.status).toBe(400); });
  test("returns 400 when email is invalid", async () => { const res = await POST(makeRequest({ name: "Phil", email: "not-valid", message: "Hi" })); expect(res.status).toBe(400); });
...
describe("POST /api/contact — success", () => {
  ...
  test("returns 200 with valid body", async () => { const res = await POST(makeRequest(validBody, "10.6.0.1")); expect(res.status).toBe(200); expect((await res.json()).ok).toBe(true); });
  test("phone is optional", async () => { const res = await POST(makeRequest({ name: "Phil", email: "phil@example.com" }, "10.6.0.2")); expect(res.status).toBe(200); });
  test("message is optional", async () => { const res = await POST(makeRequest({ name: "Phil", email: "phil@example.com" }, "10.6.0.3")); expect(res.status).toBe(200); });
  test("sanitises HTML tags", async () => {
    await POST(makeRequest({ name: "<b>Phil</b>", email: "phil@example.com", message: "Hi" }, "10.6.0.4"));
    expect(mockSend.mock.calls[0][0].text).toContain("Phil");
    expect(mockSend.mock.calls[0][0].text).not.toContain("<b>");
  });
  test("from field is correct domain", async () => {
    await POST(makeRequest(validBody, "10.6.0.5"));
    expect(mockSend.mock.calls[0][0].from).toMatch(/clear-accounts\.vercel\.app/);
  });
});
...
describe("POST /api/contact — error handling", () => {
  test("returns 500 without exposing Resend error", async () => {
    ...
    const res = await POST(makeRequest(validBody, "172.16.6.1"));
