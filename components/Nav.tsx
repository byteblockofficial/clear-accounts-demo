    ...
    <header className="sticky top-0 z-50 bg-white border-b border-[var(--color-border)] shadow-sm">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="font-bold text-lg text-[var(--color-primary)]" aria-label="Clear Accounts — home">Clear Accounts</Link>
          ...
          <a href="/contact" className="hidden md:inline-flex items-center gap-2 bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-white font-semibold text-sm px-4 py-2.5 rounded-md transition-colors">Free consultation</a>
      ...
      {open && (
        <div id="mobile-menu" className="md:hidden border-t border-[var(--color-border)] bg-white">
          <nav aria-label="Mobile navigation" className="px-4 py-3 flex flex-col gap-1">
            ...
            <a href="/contact" className="mt-2 flex items-center gap-2 bg-[var(--color-primary)] text-white font-semibold px-4 py-3 rounded-md">Free consultation</a>
          </nav>
        </div>
      )}
    </header>
