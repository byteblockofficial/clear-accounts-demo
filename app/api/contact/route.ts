...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await resend.emails.send({ from: "noreply@clear-accounts.vercel.app", to: recipientEmail, replyTo: rawEmail || undefined, subject: `New consultation request from ${name}`, text: [`Name:    ${name}`, `Email:   ${email}`, `Phone:   ${phone}`, ``, `About their business:`, message].join("\n") });
